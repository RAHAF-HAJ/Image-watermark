...
function add_watermark($image, $watermark, $dest = '')
{
    // load source image to memory
    $image = imagecreatefromfile($image);
    ...
    $destpath = 'test/'.basename($image);
    imagejpeg($image, $destpath, 100);  // use best image quality (100)
...
//add_watermark($_GET['image'], $_GET['watermark']);

echo basename('test.png');
